...
- **升级:真·3D-box DPAS na3d 原语(§3b)**:扩展补丁加了 3D-box 掩码(NATTEN shift-clamp),
  与 LTX box 语义对齐(自校验 <3e-2,不再 30 dB),真实 LTX-2.5 层 **1–7.1x** 于 LTX eager na3d
  (小空间/大 head 时最强;大 H·W 时受扁平超集所限退化到 1x)。
...
## 3b. 真·3D-box DPAS na3d 原语(升级:替代 factorized 近似)

为消除 §1b 里 factorized 的 30 dB 语义差,并把 1D DPAS 变成可复用的 **3D-box na3d**
原语(给 DiT 也用),我扩展了 sycl-tla FMHA 补丁:新增 3D-box 掩码模式(query 流式
`(t,h,w)`,按 NATTEN 的 shift-clamp 盒窗判定 `|·|`),复用整条 DPAS 管线。`natten3d_box.cpp`
自带 shift-clamp 盒参考,逐层对拍。

B70 实测(head_dim=64,真实 LTX-2.5 层形状,正确性全 PASS):

| (t,h,w,heads, box) | LTX eager na3d | 我们 DPAS 3D-box | 加速 |
|---|---:|---:|---:|
| 17,28,28, H8, 3·5·5 | 0.350 | 0.349 | **1.0x** |
| 9,14,14, H8, 3·5·5 | 0.409 | 0.847 | 2.1x |
| 5,14,14, H16, 3·7·7 | 0.831 | 2.014 | 2.4x |
| 5,7,7, H32, 3·7·7 | 0.693 | 4.947 | **7.1x** |

**语义**:我们做的是与 LTX 相同的 shift-clamp 3D 盒 → 数值与 box na3d 对齐(自校验
`max_abs_err < 3e-2`),不再是 factorized 的 30 dB 近似。
**已知局限(诚实)**:K-tile 循环用的是"盒在扁平索引下的超集窗口" `effW=(kt-1)·H·W+(kh-1)·W+(kw-1)`;
当空间 `H·W` 大时(t=17 层)超集远大于真实盒 → 多算空 K-tile → 加速退化为 1.0x。小空间/大头数时
超集接近真盒 → 高达 7.1x。彻底解决需按盒做**真三维 gather 的 K 迭代**(更复杂的 CuTe 改动),列为后续。
